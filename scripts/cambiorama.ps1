<#
  ...
  Si la rama:
    ...
    - No existe ni local ni remoto -> avisa y no hace nada (usa git-new-branch.ps1
  ...
  Uso:
    .\scripts\git-checkout-branch.ps1
    .\scripts\git-checkout-branch.ps1 -Remote origin
#>
param(
...
if ($localExists) {
    ...
    if ($remoteExists) {
        ...
        Write-Host "Usa scripts\git-new-branch.ps1 si quieres crearla con tus cambios actuales."
        exit 1
    }
}
